...
4. Create the directory `learning/<slug>/` **and its workspace skeleton** — the subfolders `lessons/`, `assignments/`, and `quizzes/` (e.g. `mkdir -p learning/<slug>/{lessons,assignments,quizzes}`). Downstream skills write into these (`teach-lesson` → `lessons/`, `create-assignment` → `assignments/`, `teach-lesson`/`assess-understanding` → `quizzes/`), so create them now rather than relying on each later write to materialize its parent.
...
- On a NEW subject: it CREATES `manifest.json` with `subject`, `slug`, `created`, `updated`, `scale`, `settings`, and the initial `current` block plus empty schema placeholders; and creates `learning/<slug>/` (with its `lessons/`, `assignments/`, `quizzes/` subfolders) and `progress.md`.
